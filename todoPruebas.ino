...
// Nueva función para detener el audio
void detenerAudio() {
  Serial.write(0x7E);
  Serial.write(0xFF);
  Serial.write(0x06);
  Serial.write(0x16);  // Comando para detener
  Serial.write(0x00);
  Serial.write(0x00);
  Serial.write(0x00);
  Serial.write(0xEF);
...
void doDispense(bool fromSchedule) {
  ...
  if (estado == HIGH) {
    ...
    // Bucle modificado para detectar tecla 'C' y detener el audio
    for(int i = 0; i < 2; i++) {
      lcdTitle("Poca comida", "Revisar tolva (C=Silencio)");
      Serial.begin(9600);
      delay(1000);
      reproducirPista(1);
      
      // Monitorear tecla 'C' durante el tiempo que suena el audio
      unsigned long audioStart = millis();
      bool audioStopped = false;
      
      while (millis() - audioStart < 4000) { // 4 segundos de audio
        char key = keypad.getKey();
        if (key == 'C' && !audioStopped) {
          detenerAudio();
          audioStopped = true;
          lcdTitle("Audio detenido", "Presiona cualquier tecla");
          
          // Esperar que el usuario presione cualquier tecla para continuar
      ...
      // Si se detuvo el audio, salir del bucle de repetición
      if (audioStopped) {
        break;
      }
    }
